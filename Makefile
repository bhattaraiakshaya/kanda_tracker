...
shell:
	@echo 'To activate the venv use source ~/.venv/kanda_tracker/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 kanda_tracker -r -d
	${bin_path}/autopep8 kanda_tracker -r -i
	@echo '---Running isort---'
	${bin_path}/isort kanda_tracker --diff
	${bin_path}/isort kanda_tracker --atomic
